...
export function LeagueTable({ league, rows, followedTeamIds }: LeagueTableProps) {
  ...
  const showDraws = !showPct && !isF1; // NRL has occasional draws, EPL regularly does

  // F1 shows constructor name column instead of W/D/L
  if (isF1) {
    return (
      <div className="glass rounded-2xl p-4">
        <p className="text-[10px] font-semibold uppercase tracking-widest text-white/30 mb-3">
          {LEAGUE_LABEL[league]} Table
        </p>
                ...
                return (
                  ...
                  >
                    ...
                    <td className={`py-1 pr-1 min-w-0 ${highlighted ? 'text-white font-bold' : 'text-white/60'}`}>
                      ...
                      {row.constructorName && (
                        <div className="text-[9px] text-white/30 truncate max-w-[80px]">
                          {row.constructorName}
                        </div>
                      )}
                    </td>
