...
async function fetchSOOGenerationFixtures(lookbackDays: number): Promise<UpcomingGame[]> {
  ...
  allSOO.forEach((e: any, idx: number) => {
    ...
    out.push({
      ...
      venue:           comp.venue?.fullName ?? '',
      // Origin venue rotation: neutral everywhere except the two genuine home
      // grounds (leave undefined there so the registered-ground match decides).
      neutralSite:     /suncorp|accor/i.test(comp.venue?.fullName ?? '') ? undefined : true,
      broadcast:       ['Nine Network', 'Fox Sports'],
