...
const SPORT_CONTEXT: Record<string, string> = {
  ...
  nba: `NBA Basketball. Use NBA-specific terminology: pick-and-roll coverage, drop vs switch, transition offense, pace, spacing, three-point volume, rim protection, off-ball movement, the paint, second unit, closing lineups, back-to-backs, load management. The table is called "the standings"; games run four 12-minute quarters (48 minutes) with a 24-second shot clock; the regular season is 82 games.`,
...
Championship points: 25/18/15/12/10/8/6/4/2/1 for positions 1–10, plus 1 point for fastest lap. The standings are called "the Championship". Use F1 terminology: active aero, Manual Override (MO), undercut, overcut, pit window, soft/medium/hard compounds, safety car, VSC, parc fermé, setup, downforce, tyre deg. Do NOT reference DRS as a current system — it does not exist in 2026.`,
  cricket_int: `International Cricket. Match the analysis to the FORMAT given (T20: powerplay, death overs, pinch-hitting, matchups; ODI: building partnerships, the middle overs, death bowling; Test: new-ball spells, sessions, declarations, follow-on, batting/bowling collapses, the fourth-innings chase). Use cricket terminology: top order, middle order, the tail, powerplay, spin vs pace, swing/seam, reverse swing, the new ball, run rate, required rate, strike rate, economy, partnerships, the toss (bat/bowl decision), pitch/wicket conditions (green, dry, turning, flat deck). Refer to the standings as a "series" or "tournament" — there is no week-to-week ladder for a bilateral series. Use "side" or "team". REGISTER BAN — this is cricket, not a football code: NEVER use "structural edge", "structures", "phases of the game", "across all phases", "gainline", "field position battle" or similar footy-analysis vocabulary; where the general instructions say "structural", read "tactical" and express it in the cricket terms above. PITCH DISCIPLINE: characterise the pitch or conditions ONLY when a VENUE PROFILE or match context in the data block supports it — never speculate ("could be a flat wicket") from nothing.`,
  bbl: `Big Bash League (BBL) — Australian domestic men's T20 franchise cricket. T20-specific: the powerplay, pinch-hitting up top, the death overs, matchup bowling, spin in the middle overs, big hitting, the run chase, net run rate for the table. Use cricket terminology: top order, finisher, the tail, strike rate, economy, the toss, pitch conditions. The competition has a league table then finals. Use "side" or "team". REGISTER BAN — this is cricket, not a football code: NEVER use "structural edge", "structures", "phases of the game", "across all phases", "gainline", "field position battle" or similar footy-analysis vocabulary; where the general instructions say "structural", read "tactical" and express it in the cricket terms above. PITCH DISCIPLINE: characterise the pitch or conditions ONLY when a VENUE PROFILE or match context in the data block supports it — never speculate ("could be a flat wicket") from nothing.`,
};
...
• DEFAULT CONSEQUENCES GO UNSAID: in a finals series, "the loser is eliminated" is what every reader already assumes — never write it, and never write its twins ("season on the line", "loser goes home") as information. State a consequence ONLY when it differs from that default: the qualifying-final loser who is NOT eliminated (the double chance), the wildcard winner who claims a place in the eight, the double chance now spent. "Win and they are in the Grand Final" carries everything; "…and the loser is eliminated" adds nothing.
• ABSENT DATA GOES UNMENTIONED: never tell the reader what information is unavailable ("no specific stakes are available", "no injury news provided", "limited data at this stage"). A gap in the data block is an instruction to YOU about what not to discuss — it is never content. Write only from what is known; a topic with no data simply does not come up.
...
REGISTER MECHANICS — how the professionals actually write it:
• NEVER announce what matters — no "The key contest will be…", "The decisive factor is…", "X will be crucial/critical/paramount". Make the case instead: "[Host] tackle harder and win more of the hitouts. The risk is rust." (structure template — never reuse its details).
• VARY the rhythm: at least one sentence under ten words in every multi-sentence field. A string of 30-word compound sentences with em-dashes reads like a machine.
• COMPARATIVES over abstractions: "wins more of the ball at the contest" beats "superior structure"; "has greater depth" is empty unless you say WHERE the depth shows up in the data.
• ONE number per claim: fold a single figure into an argument ("first innings here average 248 — par is real") rather than reading the data block aloud in sequence.
...
{
  "context": "1–3 sentences. Specific situational setup: where each side sits in this competition and what concretely is at stake in this fixture. No generic importance statements — only state stakes that are factually grounded in the data (e.g. finals position, relegation gap, cup progression). Any points total, gap, or inside/outside-the-finals claim here MUST come verbatim from DERIVED FACTS, direction included — never from the round number, the raw table, or your own arithmetic (see DERIVED FACTS BINDS THE context FIELD). If the fixture has no distinctive stakes, state the form and position plainly and move on. THE JOB OF THIS FIELD IS THE ANGLE: pick the single sharpest story the data supports and lead with it — a streak meeting the team that ended it (form + head-to-head), rest vs run (one side had the week off), an absence cluster, a double chance spent, a venue pattern. One angle, made concrete; not a tour of the situation. For FINALS fixtures: never open with the round name or a definition of the round — see NARRATIVE COMPRESSION.",
  "tacticalBattle": "2–3 sentences. When HEAD COACHES are provided in the data block, open by naming both coaches by surname and framing the contest as a clash of their systems (e.g. 'Postecoglou's high press faces Dyche's compact mid-block'). When HEAD COACHES is absent, describe the contest using team-level attribution only — no coaching names. THE JOB OF THIS FIELD IS THE MECHANISM — HOW the game is won, argued through COMPARATIVE claims in this sport's terms (who does what better than whom, per the data), the way a good match preview argues it: short, declarative, evidence-led. Do NOT announce importance ("the key contest will be…") — make the case directly. Must not restate the context field's angle or pre-empt the verdict.",
  "playerSpotlight": "REQUIRED — never return an empty string. FOR F1: lead with the FOLLOWED ENTITY's full name (the driver or constructor marked '◄ FOLLOWED' in the data block). At least 80% of this section must be directly about that followed driver/constructor — their form, this circuit's characteristics relative to their strengths, championship situation. Only mention other drivers when it directly contextualises the followed entity's own position. FOR ALL OTHER SPORTS: THE JOB OF THIS FIELD IS THE PEOPLE. If player data appears in the data block (lineup/squad/injury report/key performers/ratings), name the single most analytically compelling player from that data and connect them to the specific gamestate. If the data block contains a NO PLAYER DATA notice, pick ONE positional role and be specific about its demands in THIS game — do not fall back to re-describing the overall tactical contest (that is tacticalBattle's job; repeating it here is a defect).",
  "verdict": "2–3 sentences. The most probable outcome based on the available data, with the specific reasoning. If there is a genuine swing factor grounded in the data (an injury, a set-piece disparity, a form gap), name it. Do not add a generic hedge — if the outcome is uncertain, state why it is uncertain specifically. THE JOB OF THIS FIELD IS THE CALL: your position and the one thing that flips it. It must add something the other fields did not — never a summary of them.",
  "keyInsights": [
